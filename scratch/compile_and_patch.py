...
# List of duplicate IDs we want to uniquify
dup_ids = ["target-lang", "target-interp", "db-scope-outer", "db-scope-inner", "db-src-x", "db-src-a", "db-ref-1-0", "arrow-pair-blue"]

def remove_dom_content_loaded(script_text):
    # Find start of document.addEventListener("DOMContentLoaded"
    start_match = re.search(r'document\.addEventListener\(\s*["\']DOMContentLoaded["\']', script_text)
    if not start_match:
        return script_text
    
    start_idx = start_match.start()
    # Find the opening brace '{' after DOMContentLoaded
    open_brace_idx = script_text.find('{', start_idx)
    if open_brace_idx == -1:
        return script_text
    ...
    while depth > 0 and idx < len(script_text):
        if script_text[idx] == '{':
            depth += 1
        elif script_text[idx] == '}':
            depth -= 1
        idx += 1
    
    # Consume trailing closing parentheses and semicolon
    end_idx = idx
    if end_idx < len(script_text) and script_text[end_idx] == ')':
        end_idx += 1
    if end_idx < len(script_text) and script_text[end_idx] == ';':
        end_idx += 1
        
    return script_text[:start_idx] + script_text[end_idx:]
...
for idx, fname in enumerate(files):
    ...
    # 0. Uniquify duplicate IDs inside this volume's HTML and JS
    # Match exact duplicate ID (not part of a longer word/hyphenated string)
    pattern = r'(?<![\w-])(' + '|'.join(dup_ids) + r')(?![\w-])'
    html_content = re.sub(pattern, f'vol{vol_num}-\\1', html_content)
    ...
    for i, start_m in enumerate(tab_starts):
        ...
        # Count braces/tags to find matching closing div tag
        depth = 1
        pos = start_pos + 5
    ...
    # 4. Extract and wrap Diagram Script Logic
    scripts = re.findall(r'<script>(.*?)</script>', html_content, re.DOTALL)
    for s in scripts:
        if "function showTab" in s:
            # Clean DOMContentLoaded block
            clean_s = s
            while 'document.addEventListener("DOMContentLoaded"' in clean_s:
                clean_s = remove_dom_content_loaded(clean_s)
                
            # Rename showTab to showTab_old to prevent global redeclaration syntax error
            clean_s = clean_s.replace("function showTab(tabId) {", "function showTab_old(tabId) {")
            
            # Strip comments to avoid matching functions inside comments/documentation
            comment_free_s = re.sub(r'/\*.*?\*/|//.*?$', '', clean_s, flags=re.DOTALL | re.MULTILINE)
            
            # Find all functions defined inside this script
            funcs = re.findall(r'function\s+(\w+)\s*\(', comment_free_s)
            
            # Generate exports for global scope (window)
            exports = []
            js_keywords = {"if", "for", "while", "do", "switch", "catch", "class", "const", "let", "var", 
                           "function", "return", "throw", "try", "import", "export", "default", "with", 
                           "yield", "await", "break", "continue", "debugger", "delete", "in", "instanceof", 
                           "new", "typeof", "void", "true", "false", "null", "undefined"}
            excluded_funcs = {"showTab_old", "toggleSidebar", "updateToggleBtnIcon"} | js_keywords
            for f in funcs:
                if f not in excluded_funcs:
                    exports.append(f"window.{f} = {f};")
            
            exports_str = "\n".join(exports)
            
            # Wrap script block in a private scope to avoid variable collisions
            scoped_script = f"""
/* ==============================================
   VOLUME {vol_num} SCOPED SIMULATORS
   ============================================== */
{{
{clean_s.strip()}
...
"""
            all_diagram_scripts.append(scoped_script)
...
# Setup script (ScrollSpy, accordions, and initialization calls)
...
# Build template
continuous_html_content = f"""<!DOCTYPE html>
...
with open(output_file, "w", encoding="utf-8") as f:
    f.write(continuous_html_content)

print("SUCCESS: continuous.html compiled and patched successfully!")
print("New file size:", len(continuous_html_content), "chars")
