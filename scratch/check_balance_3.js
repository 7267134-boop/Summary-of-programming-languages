...
console.log(`\n=== Balance Check Done for 3.html ===`);
console.log(`Unclosed divs remaining in stack: ${divStack.length}`);
divStack.forEach(d => {
    console.log(`  Unclosed <div id="${d.id}" class="${d.class ? d.class.substring(0, 30) : null}"> opened at line ${d.line}`);
});
console.log(`Total unmatched closing divs: ${errorCount}`);
process.exit(divStack.length === 0 && errorCount === 0 ? 0 : 1);
