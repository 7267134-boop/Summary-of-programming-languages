(module store (lib "eopl.ss" "eopl")
  ...
  (provide initialize-store! reference? newref deref deref-guard setref!
  ...
  (define newref
    (lambda (val grd)
      (let ((next-ref (length the-store)))
        (set! the-store
              (append the-store (list (list val grd))))
  ...
  (define deref 
    (lambda (ref)
      (car (list-ref the-store ref))))

  (define deref-guard
    (lambda (ref)
      (cadr (list-ref the-store ref))))
          ...
          ((setref-inner
             ...
             (lambda (store1 ref1)
               (cond
                 ...
                 ((zero? ref1)
                  (cons (list val (cadr (car store1))) (cdr store1)))
                 (else
                   (cons
                     (car store1)
                     (setref-inner
