...
in letrec even(d) = if zero?(x) then 1 
   ...
   in let d = set x = 13 in (odd -99)" 1)
      ...
      ;; guard
      (guard-1 "let # p = 1 in p" 1)
      (guard-2 "let ? p = zero?(1) in 1" 1)
      (guard-3 "let @ p = proc(x) x in (p 1)" 1)
      (guard-4 "(proc (# x) x 1)" 1)
      (guard-5 "(proc (? x) 1 zero?(0))" 1)
      (guard-6 "(proc (@ x) 1 proc(y) y)" 1)

      ;; guard-error
      (guard-error-1 "let # q = zero?(9) in q" error)
      (guard-error-2 "let @ q = proc(# w) -(w,1) in (q zero?(9))" error)
      (guard-error-3 "let # w = 8 in set w = zero?(9)" error)
