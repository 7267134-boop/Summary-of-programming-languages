...
in ((f 44) 33)"
	12)
      ...
      ;; only one const
      (too-many-const-1 "let const const w = 8 in w" error)
      (too-many-const-2 "let p = proc(w) w in (p const const 5)" error)

      ;; const working normally
      (const-1 "let const w = 8 in w" 8)
      (const-2 "let const w = 8 in let x = w in begin set x = 60; w end" 8)
      (const-call-1 "let p = proc(w) w in (p const 5)" 5)

      ;; error when setting const
      (const-error-1 "let const w = 8 in begin set w = 60; w end" error)
      (const-error-2 "let const w = 8 in let x = w in begin set w = 60; w end" error)
      (const-call-error-1 "let p = proc(w) begin set w = 800; w end in (p const 60)" error)
