(module store (lib "eopl.ss" "eopl")
  ...
  (define reference?
    (lambda (v)
      (and (integer? (car v)) (boolean? (cadr v)))))
  ...
  (define newref
    (lambda (val is-const)
      (let ((next-ref (length the-store)))
        (set! the-store
              (append the-store (list (list val is-const))))
        (when (instrument-newref)
            (eopl:printf 
             "newref: allocating location ~s with initial contents ~s~%"
             next-ref val))                     
        (list next-ref is-const))))

  ;; deref : Ref -> ExpVal
  ;; Page 111
  (define deref 
    (lambda (ref)
      (car (list-ref the-store (car ref)))))
          ...
          ((setref-inner
             ...
             (lambda (store1 ref1)
               (cond
                 ...
                 ((zero? ref1)
                  (cons (list val #f) (cdr store1)))
                 (else
                   (cons
                     (car store1)
                     (setref-inner
                       (cdr store1) (- ref1 1))))))))
          (setref-inner the-store (car ref))))))
