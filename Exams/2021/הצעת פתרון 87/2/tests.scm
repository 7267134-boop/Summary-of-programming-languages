...
in ((f 44) 33)"
	12)
      ...
      ;; mult
      (mult-1 "*(1,10)" 10)
      (mult-2 "let x = 5 in *(x,10)" 50)

      ;; generator

      (generator-1 "let g = generator(x): [5 2 ] yield *(x,x) in -(::g,::g)" 21)
      (generator-2 "let g = generator(x): [5] yield *(x,x) in -(::g,::g)" error)
      (generator-3 "let i = 10 in let g = generator(x): [5] yield *(x,x) in -(::i,::g)" error)
      (generator-4 "
      let sum = 0
      in
      ...
      in
        (f generator(x): [5 3] yield *(x,x))" 34)
